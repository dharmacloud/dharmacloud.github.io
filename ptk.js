(() => {
  ...
  var dedup = (arr, sorted = false) => {
    if (!arr || !arr.length)
      return [];
    if (!sorted)
      arr.sort(typeof arr == "string" ? alphabetically : (a, b) => a - b);
    const out = [];
    let prev = arr[0];
  ...
  var packStrings = (sl) => {
    ...
    for (let i = 1; i < sl.length; i++) {
      ...
      if (shared && (prevhw?.codePointAt(0) || 0) < 65536) {
        out += String.fromCharCode(CodeStart2 + shared) + prevhw.substr(shared);
      } else {
        out += SEP + sl[i];
      }
    }
    ...
    match(text) {
      ...
      while (upper > 0) {
        ...
        if (text.startsWith(found))
          out.push(found);
        else if (text.codePointAt(0) < 256 || text[0] !== found[0])
          break;
        upper--;
      }
  ...
  var extractAuthor = (arr) => {
    ...
    if (typeof arr == "string")
      arr = [arr];
    arr.forEach((str) => str.replace(/．([\u3400-\u9fff\ud800-\udfff]{2,10})[〈《]/g, (m4, m1) => out.push(m1)));
    return out;
  };
  var extractBook = (arr) => {
    const out = [];
    if (typeof arr == "string")
      arr = [arr];
    arr.forEach((str) => str.replace(/[〈《]([\u3400-\u9fff\ud800-\udfff]{2,30})/g, (m4, m1) => out.push(m1)));
    return out;
  };
  ...
  var fromBase26 = (str) => {
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      if (code < 10) {
        out += String.fromCharCode(code + 48);
      } else {
        out += String.fromCharCode(code - 10 + 65);
      }
    }
  ...
  var fromChineseNumber = (str) => {
    return parseInt(str.replace(/百([二三四五六七八九])十/, "$1\u5341").replace(/百十$/, "10").replace(/百十/, "1").replace(/百$/, "00").replace(/百/, "0").replace(/一/g, "1").replace(/二/g, "2").replace(/三/g, "3").replace(/四/g, "4").replace(/五/g, "5").replace(/六/g, "6").replace(/七/g, "7").replace(/八/g, "8").replace(/九/g, "9").replace(/^十$/, "10").replace(/^十/, "1").replace(/十$/, "0").replace(/十/, "").replace(/[○〇零]/g, "0"));
  };
  ...
  var extractChineseNumber = (str, firstnum = false) => {
    let cn = "";
    for (let i = 0; i < headerWithNumber.length; i++) {
      const pat = headerWithNumber[i];
      const m4 = str.match(pat);
      if (m4)
        cn = fromChineseNumber(m4[1]);
    }
    if (!cn) {
      const m4 = str.match(/^([一二三四五六七八九十○百零]+)$/);
      if (m4)
        cn = fromChineseNumber(m4[1]);
    }
    return cn;
  ...
  var foreignNumber = (n, style) => {
    ...
    for (let i = 0; i < s.length; i++) {
      out += String.fromCharCode(s.charCodeAt(i) - 48 + base);
    }
    return out;
  };
  ...
  var packOfftagAttrs = (attrs, opts = {}) => {
    let out = "";
  ...
  function Token(text, choff, tkoff, type) {
...
\u9FD3\u9FD2`;
  ...
  var expandWildcard = (folder, pat, isDir) => {
    ...
    if (isDir)
      files = files.filter((fn) => fs.statSync(folder + fn).isDirectory());
    return files;
  };
  ...
  var makeHeader = (name2, header, pagestarts, payload) => {
  ...
  var LineBaser = class {
    ...
    async loadSection(name2, type) {
      const [from, to] = this.sectionRange(name2, type);
      if (to > from) {
    ...
    addSection(name2, type) {
      if (!name2)
        name2 = (this.header.sectionnames.length + 1).toString();
  ...
  function validate_z(offtext, tag) {
    ...
    if (text.slice(text.length - 1) == bracket)
...
`
  ...
  // compiler/linker.ts
  var makeLineBaser = async (sourcebuffers, compiler, contentGetter) => {
    ...
    for (let i = 0; i < sourcebuffers.length; i++) {
      ...
      const { name: name2, caption: caption2, errors, processed, samepage, lazy, tagdefs, textstart, sourcetype } = compiler.compiledFiles[buf.name];
      alltagdefs.push(...tagdefs);
      if (!lazy)
        lbaser.header.preload.push(name2);
    ...
    for (let tag in compiler.typedefs) {
      ...
      const name2 = "^" + tag;
      serialized && lbaser.append(serialized, { name: name2, newpage: true, samepage: true, type: "tag" });
    }
  ...
  function scanPrimaryKeys(tofind) {
    const out = [];
    if (!tofind)
      return [];
  ...
  var addTemplate = (name2, template) => {
    ...
    if (!template.getParallels)
      template.getParallels = nop;
  };
  var Templates = {};
  addTemplate("generic", {});
  ...
  // basket/inmemory.ts
  var inMemory = (lbaser2) => {
    if (!lbaser2.name)
      lbaser2.setName("inmemory");
    const ptk = new Pitaka({ inmemory: true });
    lbaser2.dump((fn, buf, page) => ptk.setPage(page, ...parseJsonp(buf)));
    poolAdd(lbaser2.name, ptk);
    return ptk;
  };
  ...
  var makeInMemoryPtk = (lbaser2, css, comimage) => {
    let sources = [], locals = [];
    let zip, redbeanbuf;
    lbaser2.dump((pagefn, buf) => {
      sources.push({ name: lbaser2.name + "/" + pagefn, content: new TextEncoder().encode(buf) });
    });
    sources.push({ name: lbaser2.name + "/accelon22.css", content: new TextEncoder().encode(css) });
    if (comimage) {
  ...
  async function loadLines(lva, noparallel = false) {
    ...
    for (let i = 0; i < divisions.length; i++) {
      ...
      for (let j2 = 0; j2 < linetexts.length; j2++) {
        ...
        const parallels = from == 0 && j2 == 0 ? ptk.template?.getParallels(ptk, lines[j2]) : [];
  ...
  var QueryAction = class extends Action {
    ...
    async run() {
      ...
      for (let i = 0; i < this.act.length; i++) {
        ...
        this.ownerdraw = {
          painter: "queryresult",
          data: { name: name2, caption: caption2, ptk, tagname, foreign, tofind, items, backref, lexicon }
        };
      }
    }
  };
  ...
  // meta/ebag.ts
  var toSeal = (str) => {
    const m4 = str.match(/s?(\d\d\d)-?(\d\d\d)/);
    if (!m4)
      return null;
    const part = parseInt(m4[1]) - 1;
    const seq = parseInt(m4[2]) - 1;
    let cp = part * 999 + seq;
    if (cp >= 65534)
      cp += 2;
    if (cp >= 131068 + 2)
    ...
    if (cp >= 196602 + 4)
      cp += 2;
    return String.fromCodePoint(655360 + cp);
  };
  var fromSeal = (ch) => {
    const cp = ch.codePointAt(0);
    if (cp < 655360 || cp >= 869152)
      return null;
    let n = cp - 655360;
    if (n >= 65534)
      n -= 2;
      ...
      for (let j2 = 0; j2 < factors.length; j2++) {
        ...
        if (typeof factor == "string") {
          if (~text.indexOf(factor)) {
            if (include?.length && !hasOneOf(text, include))
              continue;
  ...
  var stringifyChoice = (choices, groupby = 0, groupfilter = "") => {
    ...
    for (let key in choices) {
      if (key == "symtom")
        symtom2 = choices[key].join("");
      if (key == "tounge")
        tounge2 = choices[key].join("");
      if (key == "pulse")
        pulse2 = choices[key].join("");
    }
  ...
  var pb = (el, ctx) => {
    ...
    if (ctx.fn.match(/Y\d\dn\d/)) {
      if (pn.charCodeAt(0) > 64) {
        out = "^pg[" + pn.substr(0, pn.length - 1) + "]";
        ctx.compact = false;
      } else if (pn[pn.length - 1] !== "a") {
        out = "^pg[" + pn + "]";
        ctx.compact = false;
      } else {
  ...
  var cbtt = (el, ctx) => {
    ...
    if (el.children[0].name === "cb:t" && el.children[1].name === "cb:t") {
      if (lang == "pi") {
        let pi = getPali(el.children[1].innerText(true));
  ...
  var parseXMLAttribute = (attrs) => {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      out[key] = value.slice(0, value.length - 1);
    }
    return out;
  };
  ...
  // meta/cbeta.ts
  var fixJuanT = (bkno, juan, sutraline) => {
    let bk = "";
    if (juan === 1) {
  ...
  var parseBuffer = (buf, fn = "", ctx) => {
    ...
    let m4 = fn.match(/n([\dabcdefABCDEF]+)_(\d+)/);
    let bk = "", bkno = "", chunk = "";
    ...
    if (fn[0] == "T") {
      ...
      bk = "^bk" + bkno + (sutraline !== ")" ? "(" + sutraline : "");
    }
    chunk = "^ck" + juan;
    if (!ctx.teictx) {
      ctx.teictx = {
  ...
  var diffList = (IList, JList, opts = {}) => {
    ...
    const add = (m4, a = -1, b = -1) => {
      let obj;
      if (a > -1)
        obj = { m: m4, tk: I[a].tk, [attrI]: I[a][attrI], lead: I[a].lead, tail: I[a].tail };
      if (b > -1) {
        if (!obj) {
    ...
    while (i < Ilen && j2 < Jlen) {
      if (I[i].tk === J[j2].tk) {
        ...
        j2 = pj;
        if (j2 < Jlen)
          while (i < Ilen && i - pi < maxdiff) {
            i++;
            if (i < Ilen && [i].tk === J[j2].tk) {
              for (let i2 = pi; i2 < i; i2++)
                add(1, i2, -1);
              match = true;
              break;
            }
